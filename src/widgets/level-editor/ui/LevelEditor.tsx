...
import {
  ...
  generateLevel,
  LevelConfig,
} from '../../../features/basket-game/config/levels';
import {
  ITEM_CONFIGS,
  ALL_ITEM_TYPES,
} from '../../../features/basket-game/config/items';
...
import type {
  ...
  StoneSize,
  BoosterType,
} from '../../../features/basket-game/model/types';
...
export default function LevelEditor() {
  ...
  const toggleType = useCallback((type: ItemType) => {
    setConfig((prev) => {
      ...
      return {
        ...prev,
        availableTypes: next,
        spawnWeights: Object.fromEntries(next.map((t) => [t, 1])),
      };
    });
  }, []);
  ...
  const upsertNetBlocker = useCallback(
    (state: NetState, updates: { count?: number; wrapsType?: ItemType }) => {
      setConfig((prev) => {
        const current = prev.netBlockers.find(
          (blocker) => blocker.initialState === state,
        );
        const nextCount = Math.max(0, updates.count ?? current?.count ?? 0);
        const nextWrapsType =
          updates.wrapsType ?? current?.wrapsType ?? prev.availableTypes[0];
        const nextNetBlockers = prev.netBlockers.filter(
          (blocker) => blocker.initialState !== state,
        );
  ...
  const toggleNetBlockers = useCallback((enabled: boolean) => {
    setConfig((prev) => ({
      ...
      netBlockers: enabled
        ? [
            {
              wrapsType: prev.availableTypes[0],
              initialState: 'strong',
              count: 1,
            },
          ]
  ...
  const upsertStoneBlocker = useCallback((size: StoneSize, count: number) => {
    setConfig((prev) => {
      ...
      const nextStoneBlockers = prev.stoneBlockers.filter(
        (blocker) => blocker.size !== size,
      );
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-5">
          ...
          <div>
            ...
            <input
              ...
              max={2000}
              step={10}
              value={config.harvestGoalPoints}
              onChange={(e) =>
                setConfig((p) => ({
                  ...p,
                  harvestGoalPoints: parseInt(e.target.value),
                }))
              }
              className="w-full"
            />
          ...
          <div>
            <div className="text-xs font-semibold text-gray-600 mb-1">
              ...
              <b className="text-yellow-500">
                {Math.round(config.goldenSpawnChance * 100)}%
              </b>
            </div>
            <input
              type="range"
              ...
              onChange={(e) =>
                setConfig((p) => ({
                  ...p,
                  goldenSpawnChance: parseInt(e.target.value) / 100,
                }))
              }
              className="w-full"
            />
          ...
          <div>
            <div className="text-xs font-semibold text-gray-600 mb-1">
              Появление блокеров при досыпке:{' '}
              <b>{config.blockerSpawnChance.toFixed(2)}</b>
            </div>
            <input
              type="range"
              min={0}
                ...
                {config.netBlockers.length > 0 && (
                  <div className="space-y-1.5">
                    {NET_STATES.map((state) => {
                      const blocker = config.netBlockers.find(
                        (item) => item.initialState === state,
                      );
                      return (
                        <div
                          key={state}
                          className="grid grid-cols-[84px_1fr_64px] items-center gap-2"
                        >
                          ...
                          <select
                            value={
                              blocker?.wrapsType ?? config.availableTypes[0]
                            }
                            onChange={(e) =>
                ...
                {config.stoneBlockers.length > 0 && (
                  <div className="space-y-1.5">
                    {STONE_SIZES.map((size) => {
                      const blocker = config.stoneBlockers.find(
                        (item) => item.size === size,
                      );
                      return (
                        <div
                          key={size}
                          className="grid grid-cols-[84px_64px] justify-between items-center gap-2"
                        >
                          <div className="text-[11px] font-medium text-gray-600">
                            {size === 'large'
                              ? 'Большой'
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {(['watering', 'skewer', 'blender'] as BoosterType[]).map(
                (type) => {
                  const labels: Record<BoosterType, string> = {
                    watering: '💧 Лейка',
                    skewer: '🗡 Шампур',
                    blender: '🌀 Блендер',
                  };
                  return (
                    <div key={type} className="text-center">
                      ...
                      <input
                        ...
                        min={0}
                        max={5}
                        value={config.startBoosters[type] ?? 0}
                        onChange={(e) =>
                          setConfig((p) => ({
                            ...p,
                            startBoosters: {
